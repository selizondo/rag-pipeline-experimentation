...
Six charts are generated by `scripts/run_full_pipeline.py` and saved to `visualizations/`:

| File | What it shows |
|---|---|
| `metrics_heatmap.png` | Recall@5, Precision@5, MRR, NDCG@5 across all 18 configurations — color-coded matrix |
| `dimension_impact.png` | Grouped bar chart of NDCG@5 averaged per dimension (chunking / embedding / retrieval) — reveals which dimension matters most |
| `before_after.png` | Side-by-side bars comparing retrieval metrics before and after configuration improvements |
| `radar_generation.png` | Radar chart of LLM-as-Judge scores (Relevance, Accuracy, Completeness, Citation Quality) for the best config |
| `latency_distribution.png` | Histogram of query latency across all configs — identifies configurations too slow for practical use |
| `fusion_sweep.png` | NDCG@5 as a function of hybrid fusion weight α — justifies the chosen α=0.6 |
